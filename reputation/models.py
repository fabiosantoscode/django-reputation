...
class ReputationManager(models.Manager):
    ...
    def calculate_reputation_for_today(self, user):
        ...
        has gained today.
        """
        start_time = datetime.datetime.today().replace(hour = 0, minute = 0,
                                                       second = 0)
        end_time = datetime.datetime.today().replace(hour = 23, minute = 59,
                                                     second = 59)
    ...
    def log_reputation_action(self, user, originating_user, action_value,
                              target_object):
        """
        ...
        if expected_delta <= REPUTATION_MAX_GAIN_PER_DAY \
            and expected_delta >= -1 * REPUTATION_MAX_LOSS_PER_DAY:
            delta = action_value
...
class ReputationAction(TimeStampedModel):
    ...
    def __unicode__(self):
        return "%s - %d" % (self.user.username, self.value)
